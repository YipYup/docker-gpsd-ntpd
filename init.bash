...
if [[ -f /etc/default/gpsd ]]; then
    ...
    if [[ -f /etc/default/gpsd.orig ]]; then
        echo "The backup exists. Proceeding..."
        true
     else
...
if [[ -f /usr/lib/systemd/system/gpsd.socket ]]; then
    ...
    echo "Now checking if backup of gpsd socket file exists..."
    if [[ ! -f /root/.gpsd.socket.bak ]]; then
        $(which cp) /usr/lib/systemd/system/gpsd.socket /root/.gpsd.socket.bak || echo "Failed to copy /usr/lib/systemd/system/gpsd.socket to /root/.gpsd.socket.bak" && exit 1
    else
        true
    fi
else
...
if [[ -f /lib/systemd/system/ntp.service ]]; then
    ...
    if [[ ! -f /root/.ntp.service.bak ]]; then
        $(which cp) /lib/systemd/system/ntp.service /root/.ntp.service.bak || echo "Failed to copy /lib/systemd/system/ntp.service to /root/.ntp.service.bak" && exit 1
    else
        true
    fi
else
...
###############################################################################
$(which sed) -i.orig -e 's/PrivateTmp=true/#PrivateTmp=true/' /lib/systemd/system/ntp.service
